...
// ---------------------------------------------------------------------------
// All commands use pipe-delimited arguments. Optional trailing parameters are
// marked with square brackets. NodeId and GraphName occupy different positions
// across commands and are easy to transpose — see "Argument-order mistake
// detection" below for detection and reporting.
//
// SPAWN_NODE|BPPath|NodeClass|Comment|X|Y|[GraphName]
// SPAWN_NODE_IN_GRAPH|BPPath|GraphName|NodeClass|Comment|X|Y
// CONNECT_PINS|BPPath|NodeA|PinA|NodeB|PinB|[GraphName]
// DISCONNECT_PINS|BPPath|NodeA|PinA|NodeB|PinB|[GraphName]
// DELETE_NODE|BPPath|NodeId|[GraphName]
// CLEAR_NODES|BPPath|CommentMatch|[GraphName]
// GET_NODE_PINS|BPPath|NodeId|[GraphName]
// LIST_NODES|BPPath|[GraphName]
// SET_PIN_DEFAULT|BPPath|NodeId|PinName|DefaultValue
// SET_FUNCTION_REF|BPPath|NodeId|ClassName|FunctionName
// SET_EVENT_REF|BPPath|NodeId|FunctionName
// CONNECT_ANIM_PINS|BPPath|AnimGraphName|NodeA|PinA|NodeB|PinB
// GET_ANIM_NODE_PINS|BPPath|AnimGraphName|NodeId
// [... and many more; see command dispatch below ...]
//
// Argument-order mistake detection:
// A common error is passing "EventGraph" (or another graph name) as the NodeId
// argument, leaving GraphName empty. Since "EventGraph" is typically 10+ chars
// but a GUID is always exactly 32 hex characters, the code detects this by:
// (1) Checking if the NodeId argument is NOT 32 characters,
// (2) Checking if it DOES resolve as a graph name via FindGraphByName,
// and if both are true, reporting a clear error with the argument position.
// Similar detection happens for other commands that resolve NodeIds.
// ---------------------------------------------------------------------------
...
void UGraphBridgeAutomationLibrary::ExecuteAtomicCommand(const FGraphBridgeCommandContext& Context)
{
    ...
    // ------------------------------------------------------------------
    // Function & Event parameter opcodes (v1.5)
    // ------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// DeleteNode
// Command: DELETE_NODE|BPPath|NodeGUID|[GraphName]
//
// Uses FindNodeInGraph when GraphName is provided (scoped search), or
// FindNodeAnywhere when empty (broadest search). Confirmed: a node in AnimGraph
// (which lives in FunctionGraphs, same as CREATE_FUNCTION graphs) could never
// be found with the pre-v1.7 narrower FindNodeById/FindNodeByName pair
// (EventGraph only).
//
...
// its EditorStateMachineGraph sub-graph — skipping it would silently orphan
// that sub-graph in the package instead of deleting it.
//
// Returns empty string on success, "ERR: <reason>" on any failure. Detects
// the common mistake of swapping NodeId and GraphName (a 32-hex string being
// passed as the "NodeId" argument that resolves as a graph name).
// ---------------------------------------------------------------------------
FString UGraphBridgeAutomationLibrary::DeleteNode(FString BlueprintPath, FString NodeId, FString GraphName)
{
    UBlueprint* Blueprint = GetBlueprintByPath(BlueprintPath);
    if (!Blueprint)
        return FString::Printf(TEXT("ERR:Blueprint not found at '%s'"), *BlueprintPath);

    // Detect the argument-order mistake: if NodeId looks like a graph name but
    // not like a GUID, and it resolves via FindGraphByName, tell the user.
    if (NodeId.Len() != 32 && FindGraphByName(Blueprint, NodeId))
    {
        return FString::Printf(
            TEXT("ERR:No node '%s' found. Note '%s' is a graph name — DELETE_NODE takes BPPath|NodeId|GraphName (NodeId second, GraphName last)."),
            *NodeId, *NodeId);
    }
    ...
    if (!GraphName.IsEmpty())
    {
        ...
        if (!Graph)
        {
            ...
            for (UEdGraph* G : Blueprint->MacroGraphs)
                if (G) Available.Add(G->GetName());
            return FString::Printf(
                TEXT("ERR:Graph '%s' not found. Available: %s"),
                *GraphName, *FString::Join(Available, TEXT(", ")));
        }
        Node = FindNodeInGraph(Graph, NodeId);
        if (!Node)
            return FString::Printf(TEXT("ERR:No node '%s' found in graph '%s'"), *NodeId, *GraphName);
    }
    else
    {
        Node = FindNodeAnywhere(Blueprint, NodeId);
        if (!Node)
            return FString::Printf(TEXT("ERR:No node '%s' found"), *NodeId);
    }
...
FString UGraphBridgeAutomationLibrary::ClearNodes(FString BlueprintPath, FString CommentMatch, FString GraphName)
{
    UBlueprint* Blueprint = GetBlueprintByPath(BlueprintPath);
    if (!Blueprint)
        return FString::Printf(TEXT("ERR:Blueprint not found at '%s'"), *BlueprintPath);
    ...
    if (ToRemove.Num() == 0)
        return FString::Printf(TEXT("ERR:No nodes with comment containing '%s' found"), *CommentMatch);
...
FString UGraphBridgeAutomationLibrary::SetPinDefault(FString BlueprintPath,
    FString NodeId, FString PinName, FString DefaultValue)
{
    ...
    if (!Pin)
        return FString::Printf(TEXT("ERR:Pin '%s' not found on node '%s'"), *PinName, *NodeId);
...
// Function & Event parameter opcodes (v1.5)
// ---------------------------------------------------------------------------
// ADD_FUNCTION_PARAM|BlueprintPath|FunctionGraphName|Direction|Type:Name,...
// REMOVE_FUNCTION_PARAM|BlueprintPath|FunctionGraphName|Direction|ParamName
// ADD_CUSTOM_EVENT_PARAM|BlueprintPath|EventNodeId|Type:Name,...
// REMOVE_CUSTOM_EVENT_PARAM|BlueprintPath|EventNodeId|ParamName
//
// Engine facts confirmed against UE 5.8 source before writing this:
