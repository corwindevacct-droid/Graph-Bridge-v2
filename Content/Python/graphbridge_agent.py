...
Function & Event parameter opcodes (v1.5):
  ADD_FUNCTION_PARAM|BPPath|FunctionName|Direction|Type:Name,Type:Name,...
    Direction = "in" (entry node) or "out" (result node).
    Type:Name format is comma-separated, e.g. "float:Damage,int32:ComboCount"
    Creates a result node on demand if needed. Multiple result nodes are handled.
  REMOVE_FUNCTION_PARAM|BPPath|FunctionName|Direction|ParamName
    Removes a parameter by name from the specified side of the function.
  ADD_CUSTOM_EVENT_PARAM|BPPath|EventNodeId|Type:Name,Type:Name,...
    Adds parameters to a Custom Event node (must be named first via SET_CUSTOM_EVENT_NAME).
    Uses the event's node GUID, not a graph name.
  REMOVE_CUSTOM_EVENT_PARAM|BPPath|EventNodeId|ParamName
    Removes a parameter from a Custom Event node by name.
